...
labelvar = {'SST','SSS'}; %nome dada a variavel no plot
...
goship = dir([dirin 'MONITORAMENTOS/GOSHIP/']); goship = goship(3); goship = {goship.name}; %
load([dirin 'MONITORAMENTOS/GOSHIP/' goship{1}]); %carrega os dados prepreparados do programa GOSHIP
%--------------------------------------------------------------------------

crz = fieldnames(Goship); %verifica os cruzeiros disponiveis no arquivo GOSHIP prepocessado
for i = 1:size(crz,1)
    dateGO = eval(['Goship.' crz{i} '.date']); %encontra dos dados disponniveis de cada cruzeiro
    dateGO = size(dateGO,2); %pega o arquivo mais recente para delimitar a regiao dos cruzeiros
    for j = 1:size(var,2)
        disp(var{j})
        ...
        levGO = double(eval(['Goship.' crz{i} '.pres{dateGO}'])); %cria a variavel de nivel fo GOSHIP
        latGO = double(eval(['Goship.' crz{i} '.lat{dateGO}'])); latGO = repmat(latGO,size(levGO,1),1); %cria a variavel lat do GOSHIP 
        lonGO = double(eval(['Goship.' crz{i} '.lon{dateGO}'])); lonGO = repmat(lonGO,size(levGO,1),1); %cria a variavel lon do GOSHIP
        %----------------------------------------------------------------------

        dado = squeeze(dado); %organiza os dados

        for k = 1:size(dado,4)
            dadoz(:,:,k,j) = griddata(X,Y,Z,dado(:,:,:,k),lonGO,latGO,levGO); %interpolacao dos dados dos modelos para as coordenadas do GOSHIP
        end
    end
...
save(['/home/micael/Documents/PROJETOS/ModCOSTA/CMIP6/OUTPUTS/densidade_hist'], 'dens', '-v7.3'); %salva a struct gerada do diretorio de saida)
