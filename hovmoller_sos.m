...
for i = 1:size(var,2)
    ...
    cbarlim1 = floor(mindado)+.75:.25:ceil(maxdado)-.75; %cria o vetor para delimitar a colorbar
    ...
    cbarlim2 = -cbarlim2+.75:.25:cbarlim2-.75;
    ...
    ax.Ticks = [floor(cbarlim1(1)):1:ceil(cbarlim1(end))]%set da orientacao dos marcadores para o lado de fora do subplot
    ...
    for j=2:size(ndado,2) %loop para todos os VIESES (modelos)
        ...
        sp.Position = sp.Position + [-.04 -.04 .04 .06]; %expande o tamanho do subplot
        [CS,CH]=contourf(1:12,lat,bias(:,:,j),cbarlim2,'edgecolor','none'); %plot utilizando linhas de contorno
        hold on
        ...
        elseif j == size(ndado,2) 
            ...
            ax.AxisLocation = 'out'; %set dos numeros da colobar para o lado de fora do subplot
            ax.TickDirection = 'out'; %set da orientacao dos marcadores para o lado de fora do subplot
            ax.Ticks = [-3:1:3]; %set da orientacao dos marcadores para o lado de fora do subplot
        end
