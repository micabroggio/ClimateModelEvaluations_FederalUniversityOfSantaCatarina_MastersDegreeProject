...
var = {'sos'}; %variaveis processadas
labelvar = {'SSS'}; %nome dada a variavel no plot
...
for i = 1:size(var,2)
    ...
    cbarlim1 = floor(mindado):.5:ceil(maxdado); %cria o vetor para delimitar a colorbar
    l1 = length(30:.5:cbarlim1(end)); %comprimento do vetor da colorbar. Vetor adaptado
    ...
    l2 = length(-3:.5:3); %comprimento do vetor da colorbar VIES
    ...
    hold on
    [CS2,CH2]=m_contour(lon,lat,dado(:,:,1)',[0 35],'color','k','linestyle','--'); %linha de 35 de salinidade
    ...
    title(['i) ' labelvar{1} ' ORAS5']) %titulo do mapa a)
    ...
    ax.Ticks = [cbarlim1(1):2:cbarlim1(end)]; %set da orientacao dos marcadores para o lado de fora do subplot
    caxis([30 cbarlim1(end)]) %limites da colorbar
    %----------------------------------------------------------------------
    ...
    for j=2:size(ndado,2) %loop para todos os VIESES
        ...
        sp.Position = sp.Position + [-.04 -.04 .04 .06]; %expande o tamanho do subplot
        m_proj('miller','lon',[round(min(lon)) round(max(lon))],'lat',[ceil(min(lat)) floor(max(lat))]); %set para a projecao do mapa
        [CS,CH]=m_contourf(lon,lat,bias(:,:,j)',-cbarlim2:.5:cbarlim2,'edgecolor','none'); %plot utilizando linhas de contorno
        hold on
        [CS2,CH2]=m_contour(lon,lat,dado(:,:,1)',[0 35],'color','k','linestyle','--'); %linha de 35 de salinidade
        [CS2,CH2]=m_contour(lon,lat,dado(:,:,j)',[0 35],'color','k','linestyle','-'); %linha de 35 de salinidade
        ...
        elseif j == size(ndado,2) 
            ...
            ax.AxisLocation = 'out'; %set dos numeros da colobar para o lado de fora do subplot
            ax.TickDirection = 'out'; %set da orientacao dos marcadores para o lado de fora do subplot
            ax.Ticks = [-3:3]; %set da orientacao dos marcadores para o lado de fora do subplot
            ax.TickLabels = {'<-3' '-2' '-1' '0' '1' '2' '>3'}; %set o nome dos labels para a colorbar
        end
        caxis([-3 3]) %limites da colorbar    
    end
